...
class Chatroom:
    ...
    def send(self, message):
        self.client.send_chat_message(self.name, message)
    ...
    def start(self):
        ...
        while True:
            ...
            if self.responder.poll(1000):  # check if there is a chat request
                ...
                if action == 'send':
                    for chatroom, clients in self.chatrooms.items():
                        if chatroom == chatroom_name:
                            for client in clients:
                                if client != client_name:
                                    self.publisher.send_string(f"{client} {message}")
...
class Client:
    ...
    def send_chat_message(self, chatroom, message):
        if chatroom in self.chatrooms:
            self.chatrooms[chatroom].send(message)
    ...
    def start(self):
        ...
        while True:
            ...
            if task == "quit":
                ...
                if action == "join":
                    self.chatrooms[chatroom] = Chatroom(chatroom, self)
                elif action == "leave":
                    if chatroom in self.chatrooms:
                        del self.chatrooms[chatroom]
                elif action == "send":
                    message = input("Enter message: ")
                    self.send_chat_message(chatroom, message)
