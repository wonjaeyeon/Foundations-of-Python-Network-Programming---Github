...
class Server:
    def __init__(self, host, port):
        ...
        self.publisher_chat = self.context.socket(zmq.PUB) # new : publisher for chat messages
        self.publisher_chat.bind(f"tcp://*:{self.port + 3}")  # new : bind to a new port for chat messages
    ...
    def start(self):
        ...
        while True:
            ...
            if self.responder.poll(1000):  # check if there is a chat request
                ...
                client = Client(self.host, self.port)  # Create a new client
                client.name = client_name  # Set the client name
                ...
                if action == 'join':
                    ...
                    self.publisher_chat.send_string(json.dumps({"chatroom": chatroom_name, "sender": client_name,
                                                                "message": message}))  # new : publish the chat message
...
class Client:
    ...
    def receive_chat(self): # new : receive chat messages
        while True:
            msg = json.loads(self.subscriber_chat.recv_string())
            if self.chatroom == msg.get("chatroom"):  # only print the message if it's in the same chatroom
                print(f"Message from {msg.get('sender')}: {msg.get('message')}")
        ...
        while True:
            ...
            if task == "quit":
                ...
                self.requester.send_string(json.dumps(
                    {"action": action, "chatroom": chatroom, "client_name": self.name, "message": message}))
                print(self.requester.recv_string())
            # new : receive chat messages
            receive_chat_thread = threading.Thread(target=self.receive_chat) # new : create a new thread to receive chat messages
            receive_chat_thread.daemon = True # new : set the thread as daemon
            receive_chat_thread.start() # new : start the thread
